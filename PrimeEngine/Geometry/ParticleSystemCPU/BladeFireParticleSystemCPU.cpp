...
#include "BladeFireParticleSystemCPU.h"

namespace PE {

void BladeFireParticleSystemCPU::create(Vector3 spawnPos, Vector2 size)
{
	m_hParticleBufferCPU = Handle("PARTICLE_BUFFER_CPU", sizeof(ParticleBufferCPU<BladeFireParticle>));
	ParticleBufferCPU<BladeFireParticle> *ppbcpu = new(m_hParticleBufferCPU) ParticleBufferCPU<BladeFireParticle>(*m_pContext, m_arena);
	PrimitiveTypes::UInt32 n = 1;
	ppbcpu->m_values.reset(n);

	for (PrimitiveTypes::UInt32 ip = 0; ip < n; ip++)
	{
		ppbcpu->m_values.add(BladeFireParticle());
		ppbcpu->m_values[ppbcpu->m_values.m_size-1].m_curPos = spawnPos;//Vector3(0, 0.0f, -1000.0f);
		ppbcpu->m_values[ppbcpu->m_values.m_size-1].m_size.m_x = size.m_x;//100.0f;
		ppbcpu->m_values[ppbcpu->m_values.m_size-1].m_size.m_y = size.m_y;//50.0f;
