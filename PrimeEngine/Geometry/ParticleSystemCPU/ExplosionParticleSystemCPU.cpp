...
#include "ExplosionParticleSystemCPU.h"

namespace PE {
void ExplosionParticleSystemCPU::create(Vector3 spawnPos, Vector2 size, float ageOfEmitter)
{
	m_hParticleBufferCPU = Handle("PARTICLE_BUFFER_CPU", sizeof(ParticleBufferCPU<ExplosionParticle>));
	ParticleBufferCPU<ExplosionParticle> *ppbcpu = new(m_hParticleBufferCPU) ParticleBufferCPU<ExplosionParticle>(*m_pContext, m_arena);
	PrimitiveTypes::UInt32 n = 1; 
	ppbcpu->m_values.reset(n);

	for (PrimitiveTypes::UInt32 ip = 0; ip < n; ip++)
	{
		ppbcpu->m_values.add(ExplosionParticle());
		ppbcpu->m_values[ppbcpu->m_values.m_size-1].m_pos = spawnPos + float(ip) * Vector3(0.0f, 100.0f, 0.0f);
