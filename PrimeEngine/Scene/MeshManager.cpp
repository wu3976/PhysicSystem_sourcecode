...
PE::Handle MeshManager::getAsset(const char *asset, const char *package, int &threadOwnershipMask)
{
	...
	//Default/nazicar.x_carmesh_mesh.mesha
	//Default/cobbleplane.x_pplaneshape1_mesh.mesha
	//Soldier/SoldierTransform.mesha
	//
	PEINFO("MESH LOAD: %s\n", key);
	...
	else if (StringOps::endswith(asset, "mesha"))
	{
		...
		pMesh->localXmin = pbcu_ptr->dim1_min;
		pMesh->localXmax = pbcu_ptr->dim1_max;
		pMesh->localYmin = pbcu_ptr->dim2_min;
		pMesh->localYmax = pbcu_ptr->dim2_max;
		pMesh->localZmin = pbcu_ptr->dim3_min;
		pMesh->localZmax = pbcu_ptr->dim3_max;
#if PE_API_IS_D3D11
