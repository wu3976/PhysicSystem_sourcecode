...
// added
void fillbuff(float* linedata_buf, int offset, Vector3 p1, Vector3 p2, Matrix4x4 transform) {
	p1 = transform * (p1);
	p2 = transform * (p2);

	// p1
	linedata_buf[offset++] = p1.getX();
	linedata_buf[offset++] = p1.getY();
	linedata_buf[offset++] = p1.getZ();
	linedata_buf[offset++] = 1.f;
	linedata_buf[offset++] = 1.f;
	linedata_buf[offset++] = 1.f;
	// p2
	linedata_buf[offset++] = p2.getX();
	linedata_buf[offset++] = p2.getY();
	linedata_buf[offset++] = p2.getZ();
	linedata_buf[offset++] = 1.f;
	linedata_buf[offset++] = 1.f;
	linedata_buf[offset++] = 1.f;
}
static int num_bboxmesh = 0;
void DebugRenderer::createBoundingBoxMesh(float d1min, float d1max, float d2min, float d2max, float d3min, float d3max, Matrix4x4 transform) {
	PEINFO("BOUNDING BOX %d created\n", ++num_bboxmesh);
	float linedata_buf[144];
	Vector3 A(d1min, d2max, d3min), B(d1min, d2max, d3max), C(d1max, d2max, d3min), D(d1max, d2max, d3max),
		E(d1min, d2min, d3min), F(d1min, d2min, d3max), G(d1max, d2min, d3min), H(d1max, d2min, d3max);
	fillbuff(linedata_buf, 0, A, B, transform);
	fillbuff(linedata_buf, 12, E, F, transform);
	fillbuff(linedata_buf, 24, G, H, transform);
	fillbuff(linedata_buf, 36, C, D, transform);

	fillbuff(linedata_buf, 48, A, C, transform);
	fillbuff(linedata_buf, 60, B, D, transform);
	fillbuff(linedata_buf, 72, F, H, transform);
	fillbuff(linedata_buf, 84, E, G, transform);

	fillbuff(linedata_buf, 96, A, E, transform);
	fillbuff(linedata_buf, 108, B, F, transform);
	fillbuff(linedata_buf, 120, C, G, transform);
	fillbuff(linedata_buf, 132, D, H, transform);

	createLineMesh(true, transform, linedata_buf, 24, 100000.0f, 1.f);
}
