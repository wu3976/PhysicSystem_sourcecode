...
void PositionBufferCPU::ReadPositionBuffer(const char *filename, const char *package)
{
	...
	for (int i = 0; i < n * 3; i++)
	{
		...
		// added
		if (i % 3 == 0) {
			dim1_min = std::min(dim1_min, val * factor);
			dim1_max = std::max(dim1_max, val * factor);
		}
		else if (i % 3 == 1) {
			dim2_min = std::min(dim2_min, val * factor);
			dim2_max = std::max(dim2_max, val * factor);
			
		}
		else {
			dim3_min = std::min(dim3_min, val * factor);
			dim3_max = std::max(dim3_max, val * factor);
		}
	}
	if (dim2_max - dim2_min < 0.00001f) {
		dim2_min -= 0.5f; //give a thickness to the cobbleplane to simplify collision detection. 
	}
}
