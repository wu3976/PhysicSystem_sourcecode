...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	// added
	Mesh* themash = minst_ptr->getFirstParentByTypePtr<Mesh>();
	PEINFO("SHITTTTTTTT %f %f %f %f %f %f", themash->localXmin, themash->localXmax,
		themash->localYmin, themash->localYmax,
		themash->localZmin, themash->localZmax);
	if (DEBUG) {
		DebugRenderer::Instance()->createBoundingBoxMesh(
			themash->localXmin, themash->localXmax,
			themash->localYmin, themash->localYmax,
			themash->localZmin, themash->localZmax, pMainSN->m_base);
	}
	PE::Handle hPhsyComp("PhysicComponent", sizeof(PhysicComponent));
	PhysicComponent* phsyComp = new(hPhsyComp) PhysicComponent(
		*m_pContext, m_arena, hSN, true
	);
	phsyComp->addDefaultComponents();
	/*Vector3 pdata, rdata;
	PrimitiveTypes::Float32 R = std::min(
		(themash->localXmax - themash->localXmin) / 2,
		(themash->localZmax - themash->localZmin) / 2
	);

	rdata.m_x = R;

	pdata.m_x = (themash->localXmax + themash->localXmin) / 2;
	pdata.m_y = themash->localYmin + R;
	pdata.m_z = (themash->localZmax + themash->localZmin) / 2;

	phsyComp->data1 = pdata;
	phsyComp->data2 = rdata;*/
	...
	phsyComp->A = Vector3(pmin.getX(), pmin.getY(), pmin.getZ());
	phsyComp->B = Vector3(pmin.getX(), pmin.getY(), pmax.getZ());
	phsyComp->C = Vector3(pmin.getX(), pmax.getY() * 0.66f, pmin.getZ());
	phsyComp->D = Vector3(pmin.getX(), pmax.getY() * 0.66f, pmax.getZ());
	phsyComp->E = Vector3(pmax.getX(), pmin.getY(), pmin.getZ());
	phsyComp->F = Vector3(pmax.getX(), pmin.getY(), pmax.getZ());
	phsyComp->G = Vector3(pmax.getX(), pmax.getY() * 0.66f, pmin.getZ());
	phsyComp->H = Vector3(pmax.getX(), pmax.getY() * 0.66f, pmax.getZ());
	phsyComp->center = Vector3(
		(pmin.getX() + pmax.getX()) / 2,
		(pmin.getY() + pmax.getY()) / 2,
		(pmin.getZ() + pmax.getZ()) / 2
	);
	//PEINFO("SHITTTTTTTT %f %f %f %f %f %f", phsyComp->data1.getX(), phsyComp->data1.getY(),
		//phsyComp->data1.getZ(), phsyComp->data2.getX(),
		//phsyComp->data2.getY(), phsyComp->data2.getZ());
	static int allowedEvts[] = { 0 };
	pMainSN->addComponent(hPhsyComp, &allowedEvts[0]);
